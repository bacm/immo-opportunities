...
def main() -> int:
    ...
    with psycopg.connect(
        ...
        with tempfile.TemporaryDirectory(prefix="immo-rnb-") as temporary_directory:
            ...
            if archived is None:
                actual_sha = download_asset(source_url, local_path, expected_sha)
                object_key = (
                    f"DS-02/{arguments.release}/department/{arguments.department}/RNB.csv.zip"
                )
                etag = archive_asset(
                    object_store,
                    local_path,
                ...
                if sha256_file(local_path) != archived.sha256:
                    raise RuntimeError("Archived RNB asset checksum mismatch")
                raw_asset_id = archived.id
                actual_sha = archived.sha256

            importer = RnbImporter(connection)
            outcome = importer.import_archive(
                import_run_id=f"rnb:{arguments.release}:{arguments.department}",
                release_id=release_id,
                department_code=arguments.department,
                raw_asset_id=raw_asset_id,
                source_path=local_path,
                idempotency_key=f"{release_id}:{arguments.department}:buildings:{actual_sha}",
            )
