"""Archiver et importer une release départementale BD TOPO — entrée en ligne de commande.

La logique vit dans `immo_pipelines.spatial.release_import`, partagée avec l'asset Dagster
`ds04_bdtopo_release` (BUG-19).
"""
...
from immo_pipelines.spatial.release_import import BDTOPO, import_department_release
...
def main() -> int:
    ...
    with psycopg.connect(
        ...
        match_rates = BdtopoImporter(connection).method_rates(result.release_id)
    print(
        json.dumps(
            {
